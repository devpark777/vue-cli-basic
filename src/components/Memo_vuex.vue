<template>
     <v-app>
        ...
        <div class="list">
            <ul>
                <!-- <li v-for="m in memos" @click="memoOpen(m.id)" :key="m.id"> -->
                <li v-for="m in memos" :key="m.id">
                    {{m.content}}
                    <v-btn :to="{name: 'memo_vuex_update', params:{id : m.id}}"> 
                    <!-- <v-btn to="/memo_vuex_update"> -->
                        <v-icon dark>mdi-pencil</v-icon>
                    </v-btn>
                </li>
            </ul>
             <!-- <v-btn class="mx-2 add-btn" fab dark color="indigo" v-on:click="memowrite()"> -->
             <v-btn class="mx-2" fab dark color="indigo" to="/memo_vuex_add">
...
<script>
import Memo_vuex_count from './Memo_vuex_count.vue'
export default {
    components : {
        Memo_vuex_count
    },
